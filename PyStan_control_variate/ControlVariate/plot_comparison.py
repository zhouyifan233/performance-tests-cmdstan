...
def plot_comparison(old_mcmc_samples, cv_linear_mcmc_samples, cv_quad_mcmc_samples, fig_path='savefig', fig_name='figure.png', fig_size=(8, 8)):
    ...
    if dim > 1:
        ...
        ax.plot(range(0, num_samples), old_mcmc_samples, color='blue',
                   label='original mcmc samples')
        ax.plot(range(0, num_samples), cv_linear_mcmc_samples, color='red',
                   label='with linear control variate')
        ax.plot(range(0, num_samples), cv_quad_mcmc_samples, color='black',
                   label='with quadratic control variate')
        ax.legend(loc=1, fancybox=True, bbox_to_anchor=(1.0, 1.5))
        ax.set_xlabel('mcmc iteration')
        ax.set_ylabel('Parameter')
        ax.set_title('Parameter ' + str(1), loc='left')
